...
func sourceOwnershipCharacterization() throws {
    try require(
        myptgui_source_ownership_characterization_self_test() == 1,
...
func ptsProjectionCharacterization() throws {
    try require(
        myptgui_pts_projection_characterization_self_test() == 1,
...
func metalRendererPathResolutionSupportsStandaloneAppBundle() throws {
    ...
    try require(
        candidates.contains("/Users/example/Applications/MyPTGui Native.app/Contents/Resources/libmyptgui_metal.dylib"),
        "standalone app Metal lookup omitted Contents/Resources"
    )
    try require(
        candidates.contains("/Users/example/Applications/MyPTGui Native.app/Contents/Frameworks/libmyptgui_metal.dylib"),
        "standalone app Metal lookup omitted Contents/Frameworks"
    )
    try require(
        candidates.contains("/Users/example/Applications/MyPTGui Native.app/Contents/MacOS/libmyptgui_metal.dylib"),
...
func standardImageLoadFromGeneratedFixtures() throws {
    let png = try makeFixture("myptgui_native_fixture.png", type: "public.png" as CFString)
    let tiff = try makeFixture("myptgui_native_fixture.tiff", type: "public.tiff" as CFString)
...
func progressiveSourcePreviewPreservesFullSizeAndCancellation() throws {
    let png = try makeFixture(
        "myptgui_native_source_preview.png",
...
func starAsterismPreviewProducesControlPoints() throws {
    let left = try makeStarFixture("myptgui_native_stars_left.png", dx: 0, dy: 0)
    let right = try makeStarFixture("myptgui_native_stars_right.png", dx: 26, dy: -13)
    var settings = StitchSettings()
...
func adaptiveCoverageStarMatcherReportsAccepted() throws {
    let left = try makeAdaptiveCoverageStarFixture("myptgui_native_adaptive_stars_left.png", dx: 0, dy: 0)
    let right = try makeAdaptiveCoverageStarFixture("myptgui_native_adaptive_stars_right.png", dx: 24, dy: -12)
    var settings = StitchSettings()
    settings.alignmentMode = "stars"
...
func legacyVisibleStarMetricsDoNotDecideCameraSuccess() throws {
    let left = try makeStarFixture("myptgui_native_camera_gate_left.png", dx: 0, dy: 0)
    let right = try makeStarFixture("myptgui_native_camera_gate_right.png", dx: 26, dy: -13)
    var settings = StitchSettings()
    settings.alignmentMode = "stars"
...
func homographyPreviewProducesResultPixels() throws {
    let left = try makeFeatureFixture("myptgui_native_homography_left.png", xOffset: 0)
    let right = try makeFeatureFixture("myptgui_native_homography_right.png", xOffset: 140)
    let engine = try NativeEngineBridge()
    let response = try engine.runPreview(
        paths: [left, right],
...
func previewResizing() throws {
    let png = try makeFixture("myptgui_native_resize_fixture.png", type: "public.png" as CFString, width: 8, height: 4)
    var settings = StitchSettings()
...
func rawStatusReflectsLibRawAvailability() throws {
    ...
    let result = try engine.loadImages(
        paths: [URL(fileURLWithPath: "/tmp/myptgui_native_missing_raw.ARW")],
        settings: StitchSettings(),
        progress: { _ in }
    )
    let image = try requireImage(result.images.first)
...
func contactSheetResultRemainsDiagnostic() throws {
    let png = try makeFixture("myptgui_native_contact_a.png", type: "public.png" as CFString)
    let tiff = try makeFixture("myptgui_native_contact_b.tiff", type: "public.tiff" as CFString)
    let engine = try NativeEngineBridge()
...
func manifestValidationAndSummary() throws {
    let png = try makeFixture("myptgui_native_manifest_fixture.png", type: "public.png" as CFString)
    let manifestURL = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("myptgui_native_manifest.json")
...
func nativeDefaultExportHonorsFullResolutionGate() throws {
    let left = try makeStarFixture("myptgui_native_default_export_left.png", dx: 0, dy: 0, width: 640, height: 420)
    let right = try makeStarFixture("myptgui_native_default_export_right.png", dx: 30, dy: -14, width: 640, height: 420)
    let output = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("myptgui_native_default_export.tiff")
    let engine = try NativeEngineBridge()
...
func nativeExportRejectsNonTIFFOutput() throws {
    let left = try makeStarFixture("myptgui_native_reject_export_left.png", dx: 0, dy: 0, width: 640, height: 420)
    let right = try makeStarFixture("myptgui_native_reject_export_right.png", dx: 30, dy: -14, width: 640, height: 420)
    let output = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("myptgui_native_reject_export.png")
    let engine = try NativeEngineBridge()
    let preview = try engine.runPreview(
        paths: [left, right],
        settings: StitchSettings(),
...
func nativeDiagnosticTIFFExportWritesPreview() throws {
    let left = try makeFeatureFixture("myptgui_native_export_diag_left.png", xOffset: 0)
    let right = try makeFeatureFixture("myptgui_native_export_diag_right.png", xOffset: 140)
    let output = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("myptgui_native_export_diag.tiff")
    let engine = try NativeEngineBridge()
    let preview = try engine.runPreview(
        paths: [left, right],
        settings: StitchSettings(),
...
func nativeCameraStripTIFFExportRerendersPreviewGeometry() throws {
    let left = try makeStarFixture("myptgui_native_export_strip_left.png", dx: 0, dy: 0)
    let right = try makeStarFixture("myptgui_native_export_strip_right.png", dx: 26, dy: -13)
    let output = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("myptgui_native_export_strip.tiff")
    var settings = StitchSettings()
    settings.alignmentMode = "stars"
    settings.displayStretch = false
    settings.optimizeDistortion = true
...
func nativeFullresCameraStripTIFFExportReloadsAndScalesInputs() throws {
    let left = try makeStarFixture("myptgui_native_export_fullres_strip_left.png", dx: 0, dy: 0, width: 840, height: 560)
    let right = try makeStarFixture("myptgui_native_export_fullres_strip_right.png", dx: 52, dy: -26, width: 840, height: 560)
    let output = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("myptgui_native_export_fullres_strip.tiff")
    var settings = StitchSettings()
    settings.alignmentMode = "stars"
    settings.displayStretch = false
    settings.optimizeDistortion = true
    ...
    let lowBudgetOutput = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("myptgui_native_export_low_budget.tiff")
