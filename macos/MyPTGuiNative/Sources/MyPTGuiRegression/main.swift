...
enum RegressionCLI {
    ...
    static func algorithmSourcePaths(root: URL) throws -> [String] {
        ...
        let roots = [
            "macos/MyPTGuiNative/Sources/MyPTGuiEngine",
            "macos/MyPTGuiNative/Sources/MyPTGuiCore",
            "macos/MyPTGuiNative/Sources/MyPTGuiApp",
        ...
        for relativeRoot in roots {
            ...
            for case let fileURL as URL in enumerator {
                ...
                let accepted: Bool
                if relativeRoot.hasSuffix("MyPTGuiCore") || relativeRoot.hasSuffix("MyPTGuiApp") {
                    accepted = ext == "swift"
    ...
    static func runStandardImageRegression(arguments: [String]) -> Int32 {
        ...
        var outputDirectory = URL(
            fileURLWithPath: "/tmp/myptgui_standard_image_regression",
            isDirectory: true
        )
        ...
        do {
            ...
            case "drag", "export":
                ...
                guard result != nil else {
                    throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "cancellation-smoke could not create a camera preview for \(operation)"])
                }
                if operation == "export",
            ...
            let partialPaths = [exportURL.path, exportURL.path + ".myptgui-partial"]
                + urls.indices.map { exportURL.path + ".myptgui-partial.source-\($0).rgb16" }
    ...
    struct PreviewParityOptions {
        ...
        var outputDirectory = URL(fileURLWithPath: "/tmp/myptgui_native_parity_reports", isDirectory: true)
    ...
    struct ExportParitySuiteOptions {
        var outputDirectory = URL(fileURLWithPath: "/tmp/myptgui_native_export_parity_reports", isDirectory: true)
        var caseFilters: Set<String> = []
        var settingsFilters: Set<String> = []
        var orderFilters: Set<String> = []
        var renderer = "cpu"
    ...
    struct RawDecodeParityOptions {
        var outputDirectory = URL(fileURLWithPath: "/tmp/myptgui_native_raw_decode_reports", isDirectory: true)
        var halfSize = true
        var paths: [String] = []
    }
    ...
    static func runPerf(arguments: [String]) -> Int32 {
        ...
        var caseID = "7raw"
        var renderer = "cpu"
        var outputDirectory = URL(fileURLWithPath: "/tmp/myptgui_native_perf_reports", isDirectory: true)
        var paired = false
        var repetitions = 3
        var index = 1
        while index < arguments.count {
    ...
    static func runPerfSuite(
        ...
        case "7raw":
            ids = Array(6046...6052)
        default:
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "perf --case must be 3raw, 4raw, or 7raw"])
        }
        let urls = ids.map { rawDirectory.appendingPathComponent("_DSC\($0).ARW") }
        let missing = urls.filter { !FileManager.default.fileExists(atPath: $0.path) }
        guard missing.isEmpty else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "missing RAW input: \(missing.map(\.path).joined(separator: ", "))"])
        }
        try FileManager.default.createDirectory(at: outputDirectory, withIntermediateDirectories: true)
        var settings = StitchSettings()
    ...
    static func runPairedPerfSuite(
        ...
        guard repetitions >= 3 else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "paired performance gate requires at least 3 repetitions"])
        }
        var runs: [PairedPerfRepetitionReport] = []
        var failures: [String] = []
        var cpuTimes: [Double] = []
        ...
        for index in 0..<repetitions {
            ...
            guard let cpu, let metal else {
                throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "paired performance run did not produce both CPU and Metal reports"])
            }
            if !cpu.passed {
    ...
    static func runCertification(arguments: [String]) -> Int32 {
        ...
        do {
            ...
            let defaultManifest = root.appendingPathComponent(
                "macos/MyPTGuiNative/Docs/Certification/native-source-certification.json"
            )
    ...
    static func parsePreviewParityOptions(_ arguments: [String]) throws -> PreviewParityOptions {
        ...
        while index < arguments.count {
            ...
            case "--case":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-preview requires a value after --case"])
                }
                options.caseID = arguments[index]
            case "--settings":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-preview requires a value after --settings"])
                }
                options.settingsMode = arguments[index]
            case "--order":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-preview requires a value after --order"])
                }
                options.orderMode = arguments[index]
            case "--output-dir":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-preview requires a path after --output-dir"])
                }
            ...
            case "--robust-reprojection-px":
                index += 1
                guard index < arguments.count, let value = Double(arguments[index]) else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-preview requires a numeric value after --robust-reprojection-px"])
                }
                options.robustReprojectionPx = value
            case "--robust-min-pair-inliers":
                index += 1
                guard index < arguments.count, let value = Int(arguments[index]) else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-preview requires an integer value after --robust-min-pair-inliers"])
                }
                options.robustMinPairInliers = value
            case "--native-robust-reprojection-px":
                index += 1
                guard index < arguments.count, let value = Double(arguments[index]) else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-preview requires a numeric value after --native-robust-reprojection-px"])
                }
                options.nativeRobustReprojectionPx = value
            case "--native-robust-min-pair-inliers":
                index += 1
                guard index < arguments.count, let value = Int(arguments[index]) else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-preview requires an integer value after --native-robust-min-pair-inliers"])
                }
        ...
        guard options.paths.count >= 2 else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-preview requires at least two image paths"])
        }
        guard ["original", "reverse", "sorted"].contains(options.orderMode) else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-preview --order must be original, reverse, or sorted"])
        }
        return options
    }
    ...
    static func parseExportParityOptions(_ arguments: [String]) throws -> ExportParityOptions {
        ...
        while index < arguments.count {
            ...
            case "--case":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-export requires a value after --case"])
                }
                options.caseID = arguments[index]
            case "--settings":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-export requires a value after --settings"])
                }
                options.settingsMode = arguments[index]
            case "--order":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-export requires a value after --order"])
                }
                options.orderMode = arguments[index]
            case "--output-dir":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-export requires a path after --output-dir"])
                }
                options.outputDirectory = URL(fileURLWithPath: arguments[index], isDirectory: true)
            case "--renderer":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-export requires a value after --renderer"])
                }
                options.renderer = arguments[index]
            case "--bit-depth":
                index += 1
                guard index < arguments.count, let value = Int(arguments[index]) else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-export requires an integer after --bit-depth"])
                }
        ...
        guard options.paths.count >= 2 else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-export requires at least two image paths"])
        }
        guard ["original", "reverse", "sorted"].contains(options.orderMode) else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-export --order must be original, reverse, or sorted"])
        }
        guard ["auto", "cpu", "metal_fast", "metal_quality"].contains(options.renderer) else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-export --renderer must be auto, cpu, metal_fast, or metal_quality"])
        }
        guard options.bitDepth == 8 || options.bitDepth == 16 else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-export --bit-depth must be 8 or 16"])
        }
        return options
    }
    ...
    static func parseRawDecodeParityOptions(_ arguments: [String]) throws -> RawDecodeParityOptions {
        ...
        while index < arguments.count {
            ...
            case "--output-dir":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-raw-decode requires a path after --output-dir"])
                }
                options.outputDirectory = URL(fileURLWithPath: arguments[index], isDirectory: true)
            case "--full-size":
        ...
        guard !options.paths.isEmpty else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-raw-decode requires at least one RAW path"])
        }
        return options
    }
    ...
    static func comparePreviewParity(options: PreviewParityOptions) throws -> PreviewParityReport {
        ...
        guard let directResult = direct.result else {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "native direct preview did not return a result"])
        }
        ...
        guard handles.count == importURLs.count else {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "native handle preview loaded \(handles.count) of \(importURLs.count) images"])
        }
        ...
        guard let handleResult = handleResponse.result else {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "native handle preview did not return a result"])
        }
    ...
    static func compareExportParity(options: ExportParityOptions) throws -> ExportParityReport {
        ...
        guard let previewResult = preview.result else {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "native preview did not return a result for export parity"])
        }
        ...
        guard let exportedResult = exported.result else {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "native export did not return a result"])
        }
    ...
    static func compareRawDecodeParity(options: RawDecodeParityOptions) throws -> RawDecodeParityReport {
        ...
        let missing = urls.filter { !FileManager.default.fileExists(atPath: $0.path) }
        guard missing.isEmpty else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "compare-raw-decode missing RAW input: \(missing.map(\.path).joined(separator: ", "))"])
        }
    ...
    static func orderedParityURLs(paths: [String], mode: String) throws -> [URL] {
        ...
        default:
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "unsupported parity order mode: \(mode)"])
        }
    }
    ...
    static func runPreviewParitySuite(arguments: [String]) throws -> PreviewParitySuiteReport {
        ...
        guard !settingsModes.isEmpty else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-suite selected no settings modes"])
        }
        guard !orderModes.isEmpty else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-suite selected no input orders"])
        }
        let root = try repositoryRoot()
        let rawDirectory = root.appendingPathComponent("test_astro/raw", isDirectory: true)
        ...
        guard selectedCount > 0 else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-suite selected no cases"])
        }
    ...
    static func runExportParitySuite(arguments: [String]) throws -> ExportParitySuiteReport {
        ...
        guard !settingsModes.isEmpty else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-export-suite selected no settings modes"])
        }
        guard !orderModes.isEmpty else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-export-suite selected no input orders"])
        }
        ...
        guard selectedCount > 0 else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-export-suite selected no cases"])
        }
    ...
    static func parseExportParitySuiteOptions(_ arguments: [String]) throws -> ExportParitySuiteOptions {
        ...
        while index < arguments.count {
            ...
            case "--output-dir":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-export-suite requires a path after --output-dir"])
                }
                options.outputDirectory = URL(fileURLWithPath: arguments[index], isDirectory: true)
            case "--case":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-export-suite requires a value after --case"])
                }
                options.caseFilters.formUnion(splitCommaSeparated(arguments[index]))
            case "--settings":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-export-suite requires a value after --settings"])
                }
                options.settingsFilters.formUnion(splitCommaSeparated(arguments[index]))
            case "--order":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-export-suite requires a value after --order"])
                }
                options.orderFilters.formUnion(splitCommaSeparated(arguments[index]))
            case "--renderer":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-export-suite requires a value after --renderer"])
                }
                options.renderer = arguments[index]
            case "--linear-only":
            ...
            case "--resume":
                options.resume = true
            default:
                throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "unknown parity run-export-suite argument: \(argument)"])
            }
            index += 1
        }
        ...
        if !invalidSettings.isEmpty {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-export-suite --settings must contain only ui-default or camera"])
        }
        let allowedOrders = Set(["sorted", "reverse"])
        let invalidOrders = options.orderFilters.subtracting(allowedOrders)
        if !invalidOrders.isEmpty {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-export-suite --order must contain only sorted or reverse"])
        }
        guard options.renderer == "cpu" || options.renderer == "metal_quality" else {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-export-suite --renderer must be cpu or metal_quality"])
        }
        return options
    }
    ...
    static func validateFocusedRelease(
        ...
        for url in options.installationReports {
            ...
            installationLaunch = installationLaunch
                ...
                && passed(report)
                && report?["bundle_identifier"]?.stringValue == "com.zhaoxinmiao.MyPTGuiNative"
    ...
    static func validateParityRelease(
        ...
        let output = options.output ?? URL(fileURLWithPath: "/tmp/myptgui_native_parity_release_report.json")
        ...
        if let evidenceBinaryFingerprint = provenanceAudit.binaryEngineSourceFingerprint,
           evidenceBinaryFingerprint != releaseProvenance.binaryEngineSourceFingerprint {
            failures.append("release evidence embedded binary fingerprint does not match the current MyPTGuiRegression binary")
        }
        if let evidenceExecutableSHA256 = provenanceAudit.regressionExecutableSHA256,
           evidenceExecutableSHA256 != releaseProvenance.regressionExecutableSHA256 {
            failures.append("release evidence was not generated by the current MyPTGuiRegression executable")
        }
    ...
    static func parseReleaseValidationOptions(_ arguments: [String]) throws -> ReleaseValidationOptions {
        ...
        while index < arguments.count {
            ...
            case "--raw2-auto-refinement", "--raw2-manual-reoptimization", "--raw2-export-pair", "--raw1-canvas-smoke", "--ordinary-image-report", "--control-point-report", "--refinement-cancellation-report", "--projection-release-report", "--installation-report", "--output":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity validate-release requires a path after \(argument)"])
                }
                let url = URL(fileURLWithPath: arguments[index]).standardizedFileURL
                switch argument {
            ...
            default:
                throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "unknown parity validate-release argument: \(argument)"])
            }
            index += 1
        }
        return options
    ...
    static func validateReleaseProvenance(
        ...
        if sourceFingerprint != binaryEngineSourceFingerprint {
            failures.append("\(label) was generated by a stale MyPTGuiRegression binary whose embedded engine/Metal fingerprint differs from its workspace")
        }
        if !worktreeClean {
        ...
        if let expected = audit.regressionExecutableSHA256,
           expected != regressionExecutableSHA256 {
            failures.append("\(label) was produced by a different MyPTGuiRegression executable")
    ...
    static func validateExportReleaseEvidence(
        ...
        for entry in cases {
            ...
            guard let outputPath = report["native_export_path"]?.stringValue,
                  ...
                  !FileManager.default.fileExists(atPath: outputPath + ".myptgui-partial") else {
                exportCorrectness = false
    ...
    static func parsePreviewParitySuiteOptions(_ arguments: [String]) throws -> PreviewParitySuiteOptions {
        ...
        while index < arguments.count {
            ...
            case "--output-dir":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-suite requires a path after --output-dir"])
                }
                options.outputDirectory = URL(fileURLWithPath: arguments[index], isDirectory: true)
            case "--case":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-suite requires a value after --case"])
                }
                options.caseFilters.formUnion(splitCommaSeparated(arguments[index]))
            case "--settings":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-suite requires a value after --settings"])
                }
                options.settingsFilters.formUnion(splitCommaSeparated(arguments[index]))
            case "--order":
                index += 1
                guard index < arguments.count else {
                    throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-suite requires a value after --order"])
                }
                options.orderFilters.formUnion(splitCommaSeparated(arguments[index]))
            case "--resume":
                options.resume = true
            case "--reuse-python-oracle":
                options.reusePythonOracle = true
            default:
                throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "unknown parity run-suite argument: \(argument)"])
            }
            index += 1
        }
        let allowedSettings = Set(["ui-default", "camera"])
        let invalidSettings = options.settingsFilters.subtracting(allowedSettings)
        if !invalidSettings.isEmpty {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-suite --settings must contain only ui-default or camera"])
        }
        let allowedOrders = Set(["original", "sorted", "reverse"])
        let invalidOrders = options.orderFilters.subtracting(allowedOrders)
        if !invalidOrders.isEmpty {
            throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "parity run-suite --order must contain only original, sorted, or reverse"])
        }
        return options
    }
    ...
    static func runPythonOracle(
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw NSError(domain: "MyPTGuiRegression", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: "Python oracle failed: \(stderrText)"])
        }
        try writeJSON(
    ...
    static func repositoryRoot() throws -> URL {
        ...
        if let configured = environment["MYPTGUI_REPOSITORY_ROOT"], !configured.isEmpty {
            let root = URL(fileURLWithPath: configured, isDirectory: true).standardizedFileURL
            guard FileManager.default.fileExists(
                atPath: root.appendingPathComponent("macos/MyPTGuiNative/Package.swift").path
            ) || FileManager.default.fileExists(atPath: root.appendingPathComponent(".git").path) else {
                throw NSError(
                    domain: "MyPTGuiRegression",
                    code: 2,
                    userInfo: [NSLocalizedDescriptionKey: "MYPTGUI_REPOSITORY_ROOT is not a MyPTGui checkout: \(root.path)"]
                )
            }
            return root
        }
        ...
        for start in starts {
            ...
            for _ in 0..<16 {
                ...
                if FileManager.default.fileExists(
                    atPath: current.appendingPathComponent("macos/MyPTGuiNative/Package.swift").path
                ) {
                    packageFallback = packageFallback ?? current
                }
                let directPackage = current.appendingPathComponent("Package.swift")
                let directSources = current.appendingPathComponent("Sources/MyPTGuiCore", isDirectory: true)
                if FileManager.default.fileExists(atPath: directPackage.path),
                   FileManager.default.fileExists(atPath: directSources.path) {
                    ...
                    if FileManager.default.fileExists(
                        atPath: possibleRepository.appendingPathComponent("native/metal_renderer/MyPTGuiMetalRenderer.mm").path
                    ) {
        ...
        throw NSError(
            domain: "MyPTGuiRegression",
            code: 2,
            userInfo: [NSLocalizedDescriptionKey: "Could not locate repository root from the working directory, executable, or compiled source path. Set MYPTGUI_REPOSITORY_ROOT."]
        )
    }
    ...
    static func pythonOracleRoot(required: Bool) throws -> URL {
        ...
        let explicit = oracleOverrides.root
            ?? (environment["MYPTGUI_PYTHON_ORACLE_ROOT"] ?? environment["MYPTGUI_ORACLE_ROOT"]).map {
                URL(fileURLWithPath: $0).standardizedFileURL
            }
        if let explicit {
        ...
        if required {
            throw NSError(
                domain: "MyPTGuiRegression",
                code: 2,
                userInfo: [NSLocalizedDescriptionKey: "Python oracle is unavailable. Pass --oracle-root or set MYPTGUI_PYTHON_ORACLE_ROOT (MYPTGUI_ORACLE_ROOT is also accepted)."]
            )
        }
        return root
    }
    ...
    static func pythonExecutable(required: Bool) throws -> URL {
        let environment = ProcessInfo.processInfo.environment
        let explicit = oracleOverrides.pythonExecutable ?? environment["MYPTGUI_PYTHON_EXECUTABLE"]
        ...
        let candidates = [
            URL(fileURLWithPath: "/opt/homebrew/Caskroom/miniconda/base/envs/myptgui/bin/python"),
        ...
        if required {
            throw NSError(
                domain: "MyPTGuiRegression",
                code: 2,
                userInfo: [NSLocalizedDescriptionKey: "Python executable is unavailable. Pass --python-executable or set MYPTGUI_PYTHON_EXECUTABLE."]
            )
        }
    ...
    static func requirePixels(_ pixels: PixelBufferInfo?, label: String) throws -> PixelBufferInfo {
        guard let pixels, pixels.width > 0, pixels.height > 0, !pixels.data.isEmpty else {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "\(label) did not return preview pixels"])
        }
        return pixels
    }
    ...
    static func savePNG(_ pixels: PixelBufferInfo, to url: URL) throws {
        guard let provider = CGDataProvider(data: pixels.data as CFData),
            ...
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "failed to create PNG at \(url.path)"])
        }
        CGImageDestinationAddImage(destination, image, nil)
        if !CGImageDestinationFinalize(destination) {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "failed to finalize PNG at \(url.path)"])
        }
    }
    ...
    static func loadPNG(_ url: URL) throws -> PixelBufferInfo {
        guard let source = CGImageSourceCreateWithURL(url as CFURL, nil),
            ...
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "failed to load PNG at \(url.path)"])
        }
        ...
        guard ok else {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "failed to decode PNG at \(url.path)"])
        }
    ...
    static func compareTIFFRGBImages(
        ...
        guard FileManager.default.fileExists(atPath: script.path) else {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "16-bit TIFF comparator is missing at \(script.path)"])
        }
        let process = Process()
        process.executableURL = python
        process.arguments = [
        ...
        guard process.terminationStatus == 0 else {
            let stderrText = String(data: stderrData, encoding: .utf8) ?? ""
            throw NSError(domain: "MyPTGuiRegression", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: "16-bit TIFF comparator failed: \(stderrText)"])
        }
        let data = try Data(contentsOf: reportURL)
        do {
            return try JSONDecoder().decode(TIFFRGBParityMetrics.self, from: data)
        } catch {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "failed to decode TIFF RGB report at \(reportURL.path): \(error.localizedDescription)"])
        }
    }
    ...
    static func runRawDecodeComparator(
        ...
        guard FileManager.default.fileExists(atPath: script.path) else {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "RAW decode comparator is missing at \(script.path)"])
        }
        var arguments = [
            script.path,
            rawURL.path,
        ...
        guard process.terminationStatus == 0 || process.terminationStatus == 3 else {
            let stderrText = String(data: stderrData, encoding: .utf8) ?? ""
            throw NSError(domain: "MyPTGuiRegression", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: "RAW decode comparator failed: \(stderrText)"])
        }
        let data = try Data(contentsOf: reportURL)
        do {
            return try JSONDecoder().decode(RawDecodeComparatorReport.self, from: data)
        } catch {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "failed to decode RAW comparator report at \(reportURL.path): \(error.localizedDescription)"])
        }
    }
    ...
    static func gitOutput(_ arguments: [String], root: URL) throws -> String {
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw NSError(domain: "MyPTGuiRegression", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: message])
        }
    ...
    static func stableJSONString<T: Encodable>(_ value: T) throws -> String {
        ...
        guard let text = String(data: data, encoding: .utf8) else {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "failed to encode provenance configuration"])
        }
        return text
    }
    ...
    static func regressionExecutableURL() throws -> URL {
        ...
        throw NSError(
            ...
            userInfo: [
                NSLocalizedDescriptionKey:
                    "Certification evidence refused: unable to locate the running MyPTGuiRegression executable for SHA-256 binding."
            ]
        )
    }
    ...
    static func makeProvenance(
        ...
        for url in inputURLs.map(\.standardizedFileURL) {
            guard FileManager.default.fileExists(atPath: url.path) else {
                throw NSError(domain: "MyPTGuiRegression", code: 2, userInfo: [NSLocalizedDescriptionKey: "provenance input is missing: \(url.path)"])
            }
        ...
        let metalURL = root.appendingPathComponent("build/native/libmyptgui_metal.dylib")
    ...
    static func writeJSON<T: Encodable>(
        ...
        guard var report = try JSONSerialization.jsonObject(with: reportData) as? [String: Any] else {
            throw NSError(domain: "MyPTGuiRegression", code: 3, userInfo: [NSLocalizedDescriptionKey: "provenance can only be attached to JSON objects"])
        }
    ...
    static func printUsage() {
        fputs(
            """
            Usage:
              MyPTGuiRegression [--oracle-root path] [--python-executable path] <command> ...
              MyPTGuiRegression capabilities
              MyPTGuiRegression dependencies
              MyPTGuiRegression load-images [--preview-max-side px] [--full-input-preview] [--raw-full-size] <image1> <image2> [...]
              MyPTGuiRegression preview [--alignment-mode auto|sift|stars] [--geometry auto|camera|homography] [--preview-max-side px] [--optimize-distortion] [--no-guided-refinement] [--no-local-refinement] [--no-texture-refinement] [--multiband] <image1> <image2> [...]
              MyPTGuiRegression standard-image-regression --case synthetic-crops|test_1_parts [--reference image] [--output-dir dir] [--max-pair-p95 px] [--min-ssim value] [--max-seconds value] [--preview-max-side px] [<image1> <image2> ...]
            ...
            For --case synthetic-crops, omit both --reference and image paths to generate a deterministic native three-crop fixture in --output-dir. Explicit paths plus --reference override generation.
              MyPTGuiRegression preview-export-diagnostic --output <preview.tiff> [--report response.json] [--bit-depth 8|16] [--strip-render|--fullres-render|--renderer backend] [--max-output-side px] [--max-output-pixels n] [--camera] [--optimize-distortion] [--no-local-refinement] [--multiband] <image1> <image2> [...]
              MyPTGuiRegression projection-drag-smoke --case 3raw|4raw|7raw [--pitch deg] [--yaw deg] [--roll deg] [--samples n] [--output report.json] [--no-blend-after-drag] [--optimize-distortion] [--multiband] <image1> <image2> [...]
              MyPTGuiRegression raw1-canvas-smoke --output report.json [--pitch deg] [--yaw deg] [--roll deg] <raw1> ... <raw7>
              MyPTGuiRegression cancellation-smoke [--operation preview|drag|export] [--working-set-budget-mb n] [--output report.json] <image1> <image2> [...]
              MyPTGuiRegression external-working-set-calibration --output report.json [--expected-exit-code n] -- <MyPTGuiRegression command and arguments>
              MyPTGuiRegression rerender-control-points --case camera-reopt|homography-reopt|disconnected [--output report.json] [--optimize-distortion] [--multiband] [--expect-disconnected] <image1> <image2> [...]
              MyPTGuiRegression raw2-auto-refinement --output report.json [--lens-profile profile.lcp] <Z8L_2185.NEF> ... <Z8L_2171.NEF>
              MyPTGuiRegression raw2-manual-reopt --fixture fixture.json [--output report.json] [--lens-profile profile.lcp] <Z8L_2185.NEF> ... <Z8L_2171.NEF>
              MyPTGuiRegression raw2-export-pair --output report.json [--output-dir dir] [--lens-profile profile.lcp] <Z8L_2185.NEF> ... <Z8L_2171.NEF>
              MyPTGuiRegression refine-astro-result --input draft-result.json --output refinement-report.json
              MyPTGuiRegression manifest validate <manifest.json>
              MyPTGuiRegression manifest summarize <manifest.json>
              MyPTGuiRegression manifest run-native [--allow-unproven-parity] [--strip-render|--fullres-render] [--output-dir dir] [--max-output-side px] [--max-output-pixels n] [--preview-max-side px] [--optimize-distortion] [--no-local-refinement] [--multiband] <manifest.json>
              MyPTGuiRegression parity compare-preview [--case id] [--settings ui-default|camera] [--order original|sorted|reverse] [--output-dir dir] [--reuse-python-oracle] <image1> <image2> [...]
              MyPTGuiRegression parity compare-export [--case id] [--settings ui-default|camera] [--order original|sorted|reverse] [--output-dir dir] [--renderer cpu|metal_fast|metal_quality] [--bit-depth 8|16] [--apply-preview-stretch] <image1> <image2> [...]
              MyPTGuiRegression parity compare-raw-decode [--output-dir dir] [--full-size] <raw1> [raw2 ...]
              MyPTGuiRegression parity run-suite [--output-dir dir] [--case id[,id...]] [--settings ui-default|camera] [--order original|sorted|reverse] [--resume] [--reuse-python-oracle]
              MyPTGuiRegression parity run-export-suite [--output-dir dir] [--case id[,id...]] [--settings ui-default|camera] [--order sorted|reverse] [--renderer cpu|metal_quality] [--linear-only] [--resume]
              MyPTGuiRegression parity validate-release [--candidate-certification] --raw2-auto-refinement raw2-auto.json --raw2-manual-reoptimization raw2-manual.json --raw2-export-pair raw2-export.json --raw1-canvas-smoke raw1-canvas.json --ordinary-image-report ordinary.json --control-point-report control-points.json --refinement-cancellation-report refinement-cancel.json --projection-release-report projection-release.json --installation-report install-launch.json [--output report.json]
              MyPTGuiRegression certification status [--manifest manifest.json] [--output report.json]
              MyPTGuiRegression certification verify [--manifest manifest.json] [--output report.json]
              MyPTGuiRegression certification promote --release-report report.json [--manifest manifest.json] [--output manifest.json]
              MyPTGuiRegression perf run-suite [--case 3raw|4raw|7raw] [--renderer cpu|metal_fast|metal_quality] [--paired --repetitions 3] [--output-dir dir]
              MyPTGuiRegression metrics-from-python-report <report.json>
              MyPTGuiRegression metrics-from-native-result <native-result.json>
              MyPTGuiRegression compare <baseline-metrics.json> <candidate-metrics.json>

            Exit code 3 means PanoLume built and ran but a required quality or release gate remains closed.

            Python parity commands also accept MYPTGUI_PYTHON_ORACLE_ROOT
            (or MYPTGUI_ORACLE_ROOT) and MYPTGUI_PYTHON_EXECUTABLE. Set
            MYPTGUI_REPOSITORY_ROOT only for a relocated standalone binary.
