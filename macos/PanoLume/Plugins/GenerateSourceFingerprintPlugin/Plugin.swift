...
struct GenerateSourceFingerprintPlugin: BuildToolPlugin {
    ...
    func createBuildCommands(context: PluginContext, target: Target) async throws -> [Command] {
        ...
        let sourceRoots = [
            repositoryRoot.appending(path: "macos/PanoLume/Sources/PanoLumeEngine"),
            repositoryRoot.appending(path: "macos/PanoLume/Sources/PanoLumeCore"),
            repositoryRoot.appending(path: "macos/PanoLume/Sources/PanoLumeApp"),
        ...
        let environmentInput = repositoryRoot.appending(path: "scripts/Private/environment.sh")
        if FileManager.default.fileExists(atPath: environmentInput.path) {
            inputs.append(environmentInput)
        }
