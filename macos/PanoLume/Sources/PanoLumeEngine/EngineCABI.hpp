// PanoLume C ABI facade implementation. Included exactly once by PanoLumeEngine.mm.

PanoLumeContext *panolume_create_context(void) {
    return new PanoLumeContext();
}
...
#if PANOLUME_HAS_OPENCV_HEADERS && PANOLUME_HAS_LIBTIFF_HEADERS
    if (!context) {
...
#if PANOLUME_HAS_OPENCV_HEADERS && PANOLUME_HAS_CERES_HEADERS
...
#if PANOLUME_HAS_LIBTIFF_HEADERS
    if (outputPath.empty()) {
...
static bool write_camera_strip_tiff_diagnostic(
    ...
    PanoLumeContext *streamingContext = nullptr,
    NativeWorkingSetStats *workingSetStats = nullptr,
    uint64_t workingSetBudgetBytes = 0
) {
#if PANOLUME_HAS_LIBTIFF_HEADERS && PANOLUME_HAS_OPENCV_HEADERS
    if (outputPath.empty()) {
        errorMessage = "output path is empty";
        return false;
    }
...
PanoLumePixelBuffer panolume_copy_image_rgba(
    ...
    PanoLumePixelBuffer empty = {nullptr, 0, 0, 0, 0};
...
PanoLumeFloatPixelBuffer panolume_copy_image_linear_rgb(
    ...
    PanoLumeFloatPixelBuffer empty = {nullptr, 0, 0, 0, 0};
    if (!context || !imageHandle) {
        return empty;
    }
    std::lock_guard<std::mutex> operationLock(context->operationMutex);
    ...
    PanoLumeFloatPixelBuffer buffer;
    buffer.width = image.width;
    buffer.height = image.height;
    buffer.channels = 3;
...
static PanoLumePixelBuffer pixel_buffer_from_result(
    NativeResult &result,
    const char *requestJson
) {
    PanoLumePixelBuffer empty = {nullptr, 0, 0, 0, 0};
