...
public enum RegressionProvenanceSchema {
    ...
    public static let toolVersion = "PanoLumeRegression-provenance-v4"
}
...
public enum RegressionEvidenceBindingError: Error, Equatable, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .missingBinarySourceFingerprint:
            return "Certification evidence refused: the running native engine does not expose its embedded source fingerprint. Rebuild PanoLumeRegression."
        case .malformedBinarySourceFingerprint:
            return "Certification evidence refused: the running native engine exposes a malformed embedded source fingerprint. Rebuild PanoLumeRegression."
        case .sourceFingerprintMismatch(let workspace, let binary):
            return "Certification evidence refused: workspace engine/Metal fingerprint \(workspace) does not match the running binary fingerprint \(binary). Rebuild PanoLumeRegression from the current workspace."
        case .missingRegressionExecutableSHA256:
            return "Certification evidence refused: the running PanoLumeRegression executable could not be hashed."
        case .malformedRegressionExecutableSHA256:
            return "Certification evidence refused: the PanoLumeRegression executable SHA-256 is malformed."
        }
    }
}
