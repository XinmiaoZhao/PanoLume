...
static constexpr int kMyPTGuiMetalAPIVersion = 5;
...
extern "C" int myptgui_metal_api_version(void) {
    return kMyPTGuiMetalAPIVersion;
}

extern "C" int myptgui_metal_is_available(void) {
    return ensure_metal() ? 1 : 0;
}

extern "C" const char *myptgui_metal_last_error(void) {
    if (!g_last_error) {
        return "";
    }
...
extern "C" void myptgui_metal_reset_interactive_cache(void) {
    @synchronized([NSCache class]) {
        [g_interactive_input_buffers removeAllObjects];
    }
}

extern "C" void myptgui_metal_begin_interactive_session(uint64_t session_id) {
    @synchronized([NSCache class]) {
...
extern "C" void myptgui_metal_end_interactive_session(uint64_t session_id) {
    @synchronized([NSCache class]) {
...
static int render_camera_strip_impl(const MyPTGuiMetalRenderRequest *request, bool quality) {
    @autoreleasepool {
        if (!request) {
...
extern "C" int myptgui_metal_render_camera_strip(const MyPTGuiMetalRenderRequest *request) {
    return render_camera_strip_impl(request, false);
}

extern "C" int myptgui_metal_render_camera_strip_quality(const MyPTGuiMetalRenderRequest *request) {
